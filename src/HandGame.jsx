...
function ChatPanel({players,rounds,totals,rules,cur}){
  ...
  const send=async t=>{if(!t.trim()||ld)return;setMsgs(p=>[...p,{r:"user",t:t.trim()}]);setInp("");setLd(true);const reply=await askAI(t.trim(),buildCtx(players,rounds,totals,rules,cur));setMsgs(p=>[...p,{r:"ai",t:reply}]);setLd(false);setTimeout(()=>speakSafe(reply),200);};
...
function ScoringModal({players,rules,curRound,onSubmit,onCancel}){
  ...
  const[farshPs,setFarshPs]=useState([]); // multi farsh players with scores
  const[stolenP,setStolenP]=useState(null);
  const[farsh2,setFarsh2]=useState(false);const[farsh2P,setFarsh2P]=useState(null);const[farsh2Score,setFarsh2Score]=useState("");
  const[handP,setHandP]=useState(null);const[penP,setPenP]=useState(null);
  const[handMode,setHandMode]=useState("normal"); // normal, super, superJoker
  ...
  const toggleFarshP=i=>{setFarshPs(prev=>prev.find(f=>f.i===i)?prev.filter(f=>f.i!==i):[...prev,{i,score:""}]);};
  const setFarshPScore=(i,v)=>{setFarshPs(prev=>prev.map(f=>f.i===i?{...f,score:v}:f));};
  const isFarshP=i=>!!farshPs.find(f=>f.i===i);

  const calc=()=>{const r={};
    if(type==="farsh"&&farshPs.length>0){
      const m=handMode==="super"?2:1;
      const farshIdxs=farshPs.map(f=>f.i);
      players.forEach((_,i)=>{
        if(farshIdxs.includes(i)){
          const fp=farshPs.find(f=>f.i===i);
          const customScore=parseInt(fp.score);
          r[i]=(!isNaN(customScore)?customScore:rules.farshScore)*m;
        }else if(i===stolenP)r[i]=rules.farshStolenScore*m;
        else if(farsh2&&i===farsh2P)r[i]=(parseInt(farsh2Score)||0)*m;
  ...
  return(<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}>
    <div style={{width:"100%",maxWidth:420,maxHeight:"90vh",overflowY:"auto",background:"linear-gradient(135deg,#1a1a3e,#24243e)",borderRadius:18,padding:18,border:"1px solid rgba(255,255,255,0.1)",direction:"rtl",color:"#fff"}}>
      ...
      {!free&&<><L>نوع الجولة</L><div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:6}}>
        <button onClick={()=>{setType("normal");setHandMode("normal");}} style={{padding:"5px 11px",borderRadius:8,border:"1px solid "+(type==="normal"?"#457B9D":"rgba(255,255,255,0.08)"),background:type==="normal"?"rgba(69,123,157,0.18)":"transparent",color:type==="normal"?"#fff":"rgba(255,255,255,0.55)",fontSize:11,fontWeight:600,cursor:"pointer"}}>عادية</button>
        {canFarsh&&<button onClick={()=>{setType("farsh");setHandMode("normal");}} style={{padding:"5px 11px",borderRadius:8,border:"1px solid "+(type==="farsh"?"#E9C46A":"rgba(255,255,255,0.08)"),background:type==="farsh"?"rgba(233,196,106,0.13)":"transparent",color:type==="farsh"?"#E9C46A":"rgba(255,255,255,0.55)",fontSize:11,fontWeight:600,cursor:"pointer"}}>فرش 🃏</button>}
      ...
      {type==="hand"&&<div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:8}}>
        <button onClick={()=>setHandMode("normal")} style={{padding:"5px 10px",borderRadius:7,border:"1px solid "+(handMode==="normal"?"#2A9D8F":"rgba(255,255,255,0.07)"),background:handMode==="normal"?"rgba(42,157,143,0.12)":"transparent",color:handMode==="normal"?"#2A9D8F":"rgba(255,255,255,0.45)",fontSize:10,fontWeight:600,cursor:"pointer"}}>هند عادي</button>
        <button onClick={()=>setHandMode("super")} style={{padding:"5px 10px",borderRadius:7,border:"1px solid "+(handMode==="super"?"#E9C46A":"rgba(255,255,255,0.07)"),background:handMode==="super"?"rgba(233,196,106,0.12)":"transparent",color:handMode==="super"?"#E9C46A":"rgba(255,255,255,0.45)",fontSize:10,fontWeight:600,cursor:"pointer"}}>🔥 سوبر دبل</button>
        <button onClick={()=>setHandMode("superJoker")} style={{padding:"5px 10px",borderRadius:7,border:"1px solid "+(handMode==="superJoker"?"#E63946":"rgba(255,255,255,0.07)"),background:handMode==="superJoker"?"rgba(230,57,70,0.12)":"transparent",color:handMode==="superJoker"?"#E63946":"rgba(255,255,255,0.45)",fontSize:10,fontWeight:600,cursor:"pointer"}}>🃏 سوبر جوكر</button>
      </div>}

      {/* Farsh super toggle */}
      {type==="farsh"&&<div style={{marginBottom:6}}><button onClick={()=>setHandMode(handMode==="super"?"normal":"super")} style={{width:"100%",padding:6,borderRadius:7,border:"1px solid "+(handMode==="super"?"rgba(233,196,106,0.25)":"rgba(255,255,255,0.06)"),background:handMode==="super"?"linear-gradient(135deg,rgba(230,57,70,0.08),rgba(233,196,106,0.08))":"transparent",color:handMode==="super"?"#E9C46A":"rgba(255,255,255,0.4)",fontSize:10,fontWeight:600,cursor:"pointer"}}>{handMode==="super"?"🔥 سوبر دبل مفعّل":"سوبر دبل"}</button></div>}

      {/* FARSH details */}
      {type==="farsh"&&<>
        <L>مين فرش؟ (اختر لاعب أو أكثر)</L>
        <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:4}}>{players.map((_,i)=><button key={i} onClick={()=>toggleFarshP(i)} style={{padding:"5px 10px",borderRadius:7,border:"1px solid "+(isFarshP(i)?players[i].color:"rgba(255,255,255,0.09)"),background:isFarshP(i)?players[i].color:"rgba(255,255,255,0.02)",color:isFarshP(i)?"#fff":"rgba(255,255,255,0.55)",fontSize:11,fontWeight:600,cursor:"pointer"}}>{players[i].name}{isFarshP(i)?" ✓":""}</button>)}</div>
        {farshPs.length>0&&<div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:6}}>
          {farshPs.map(fp=><div key={fp.i} style={{display:"flex",alignItems:"center",gap:6,background:"rgba(255,255,255,0.02)",borderRadius:7,padding:"5px 8px",border:"1px solid rgba(255,255,255,0.04)"}}>
            <div style={{display:"flex",alignItems:"center",gap:3,minWidth:60}}><div style={{width:5,height:5,borderRadius:"50%",background:players[fp.i].color}}/><span style={{fontSize:10,color:"rgba(255,255,255,0.55)"}}>{players[fp.i].name}</span></div>
            <input type="number" inputMode="numeric" placeholder={rules.farshScore+""} value={fp.score} onChange={e=>setFarshPScore(fp.i,e.target.value)} style={{flex:1,padding:4,borderRadius:5,border:"1px solid rgba(255,255,255,0.07)",background:"rgba(255,255,255,0.03)",color:"#2A9D8F",fontSize:13,fontWeight:700,textAlign:"center",outline:"none",minWidth:0}}/>
          </div>)}
        </div>}
        <L>منين انأخذت؟</L>
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
          {players.map((_,i)=>!isFarshP(i)&&<PB key={i} i={i} sel={stolenP} fn={setStolenP}/>)}
          <button onClick={()=>setStolenP(null)} style={{padding:"5px 10px",borderRadius:7,border:"1px solid rgba(255,255,255,0.08)",background:stolenP===null?"rgba(255,255,255,0.08)":"transparent",color:"rgba(255,255,255,0.5)",fontSize:10,cursor:"pointer"}}>ما أحد</button>
        </div>
        <L>فيه أحد فرش ثاني؟</L>
        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
          <button onClick={()=>{setFarsh2(!farsh2);if(farsh2){setFarsh2P(null);setFarsh2Score("");}}} style={{padding:"5px 12px",borderRadius:7,border:"1px solid "+(farsh2?"#F4A261":"rgba(255,255,255,0.08)"),background:farsh2?"rgba(244,162,97,0.12)":"transparent",color:farsh2?"#F4A261":"rgba(255,255,255,0.45)",fontSize:10,fontWeight:600,cursor:"pointer"}}>{farsh2?"نعم ✅":"لا"}</button>
        </div>
        {farsh2&&<div style={{background:"rgba(255,255,255,0.02)",borderRadius:8,padding:8,marginTop:6,border:"1px solid rgba(255,255,255,0.04)"}}>
          <L>مين فرش ثاني؟</L>
          <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{players.map((_,i)=>!isFarshP(i)&&<PB key={i} i={i} sel={farsh2P} fn={setFarsh2P}/>)}</div>
          <L>كم بيده؟</L>
          <input type="number" inputMode="numeric" value={farsh2Score} onChange={e=>setFarsh2Score(e.target.value)} placeholder="النقاط" style={{width:80,padding:5,borderRadius:6,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",color:"#fff",fontSize:13,fontWeight:700,textAlign:"center",outline:"none"}}/>
        </div>}
      </>}
      ...
      {(type==="normal"||free)&&<><L>نقاط كل لاعب</L><div style={{display:"flex",flexDirection:"column",gap:6}}>{players.map((p,i)=><div key={i} style={{background:"rgba(255,255,255,0.02)",borderRadius:8,padding:"8px 10px",border:"1px solid rgba(255,255,255,0.03)",display:"flex",alignItems:"center",gap:8}}>
        <div style={{display:"flex",alignItems:"center",gap:4,minWidth:70}}><div style={{width:6,height:6,borderRadius:"50%",background:p.color,flexShrink:0}}/><span style={{fontSize:11,color:"rgba(255,255,255,0.6)",fontWeight:600}}>{p.name}</span></div>
        <div style={{display:"flex",gap:4,alignItems:"center",flex:1}}>
          <button onClick={()=>setNeg({...neg,[i]:!neg[i]})} style={{width:28,height:28,borderRadius:6,border:"1px solid "+(neg[i]?"rgba(42,157,143,0.4)":"rgba(230,57,70,0.3)"),background:neg[i]?"rgba(42,157,143,0.12)":"rgba(230,57,70,0.08)",color:neg[i]?"#2A9D8F":"#E63946",fontSize:14,fontWeight:800,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{neg[i]?"−":"+"}</button>
          <input type="number" inputMode="numeric" placeholder="0" value={man[i]||""} onChange={e=>setMan({...man,[i]:e.target.value})} style={{flex:1,padding:"6px 4px",borderRadius:6,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",color:neg[i]?"#2A9D8F":"#fff",fontSize:16,fontWeight:700,textAlign:"center",outline:"none",boxSizing:"border-box",minWidth:0}}/>
        </div>
      </div>)}</div></>}

      {/* Preview */}
      {(type!=="normal"||Object.values(man).some(v=>v))&&<div style={{background:"rgba(255,255,255,0.02)",borderRadius:8,padding:8,margin:"8px 0",border:"1px solid rgba(255,255,255,0.03)"}}>
        <div style={{fontSize:10,fontWeight:600,color:"rgba(255,255,255,0.4)",marginBottom:3}}>معاينة {handMode==="super"?"🔥":handMode==="superJoker"?"🃏🔥":""}</div>
        {players.map((p,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"2px 0"}}><div style={{display:"flex",alignItems:"center",gap:3}}><div style={{width:5,height:5,borderRadius:"50%",background:p.color}}/><span style={{fontSize:11}}>{p.name}</span></div><span style={{fontSize:13,fontWeight:700,color:pv[i]<0?"#2A9D8F":pv[i]>=200?"#E63946":"#fff"}}>{pv[i]>0?"+":""}{pv[i]||0}</span></div>)}
      </div>}
...
export default function HandGame(){
  ...
  const submitRound=rd=>{
    ...
    const needs=p=>{const d=p.t-leader.t;if(d<=0)return null;const txt=p.name+" فارق "+d+" عن المتصدر. ";if(d<=130)return txt+"يحتاج فرش عادي";if(d<=260)return txt+"يحتاج هند عادي أو فرش دبل";if(d<=520)return txt+"يحتاج هند سوبر";if(d<=1040)return txt+"يحتاج هند سوبر جوكر";return txt+"صعب يلحق حتى بسوبر جوكر";};
  ...
  return(<div style={base}>
    ...
    <div style={{maxWidth:600,margin:"0 auto 8px",background:"rgba(255,255,255,0.04)",borderRadius:13,padding:12,border:"1px solid rgba(255,255,255,0.05)"}}>
      ...
      {sorted.map((p,rank)=>{const ld=rank===0&&rounds.length>0;return(<div key={p.idx} style={{marginBottom:5,padding:"7px 8px",borderRadius:8,background:ld?"rgba(42,157,143,0.06)":"rgba(255,255,255,0.015)",...(ld?{border:"1px solid rgba(42,157,143,0.08)"}:{}),transition:"all 0.5s ease",animation:"slideIn 0.3s ease"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:5}}><span style={{fontSize:13,fontWeight:700,minWidth:18,textAlign:"center"}}>{ld?"👑":rank+1}</span><div style={{width:7,height:7,borderRadius:"50%",background:p.color}}/><span style={{fontSize:12,fontWeight:600}}>{p.name}</span></div><span style={{fontSize:17,fontWeight:800,color:p.tot<0?"#2A9D8F":"#fff"}}>{p.tot}</span></div></div>);})}
    </div>
    ...
    {rounds.length>0&&<div style={{maxWidth:600,margin:"0 auto 14px",background:"rgba(255,255,255,0.02)",borderRadius:12,padding:12,border:"1px solid rgba(255,255,255,0.03)"}}>
      ...
      <div style={{overflowX:"auto"}}>
        ...
        {rounds.map((rd,ri)=><div key={ri} style={{display:"flex",padding:"2px 0",...(ri%2===0?{background:"rgba(255,255,255,0.01)"}:{})}}>
          ...
          {rules.scoringMode==="rules"&&<span style={{flex:"0 0 36px",textAlign:"center",fontSize:8,color:"rgba(255,255,255,0.3)"}}>{rd.type==="hand"?(rd.isSuperJoker?"جوكر🃏":rd.isSuper?"هند🔥":"هند"):rd.type==="farsh"?(rd.isSuper?"فرش🔥":"فرش"):"—"}</span>}
