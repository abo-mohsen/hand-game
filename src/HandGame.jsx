...
function ScoringModal({players,rules,curRound,onSubmit,onCancel}){
  ...
  const calc=()=>{const r={};
    if(type==="farsh"&&farshP!==null){
      const m=handMode==="super"?2:1;
      players.forEach((_,i)=>{
        if(i===farshP)r[i]=rules.farshScore*m;
  ...
  return(<div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.7)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}>
    <div style={{width:"100%",maxWidth:420,maxHeight:"90vh",overflowY:"auto",background:"linear-gradient(135deg,#1a1a3e,#24243e)",borderRadius:18,padding:18,border:"1px solid rgba(255,255,255,0.1)",direction:"rtl",color:"#fff"}}>
      ...
      {type==="farsh"&&<>
        <L>مين فرش؟</L>
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{players.map((_,i)=><PB key={i} i={i} sel={farshP} fn={setFarshP}/>)}</div>
        <L>منين انأخذت؟</L>
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
          {players.map((_,i)=>i!==farshP&&<PB key={i} i={i} sel={stolenP} fn={setStolenP}/>)}
        ...
        {farsh2&&<div style={{background:"rgba(255,255,255,0.02)",borderRadius:8,padding:8,marginTop:6,border:"1px solid rgba(255,255,255,0.04)"}}>
          <L>مين فرش ثاني؟</L>
          <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{players.map((_,i)=>i!==farshP&&<PB key={i} i={i} sel={farsh2P} fn={setFarsh2P}/>)}</div>
          <L>كم بيده؟</L>
      ...
      {(type==="normal"||free)&&<><L>نقاط كل لاعب</L><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(100px,1fr))",gap:5}}>{players.map((p,i)=><div key={i} style={{background:"rgba(255,255,255,0.02)",borderRadius:8,padding:6,border:"1px solid rgba(255,255,255,0.03)"}}>
        <div style={{display:"flex",alignItems:"center",gap:3,marginBottom:3}}><div style={{width:5,height:5,borderRadius:"50%",background:p.color}}/><span style={{fontSize:10,color:"rgba(255,255,255,0.5)"}}>{p.name}</span></div>
        <div style={{display:"flex",gap:3,alignItems:"center"}}>
          <button onClick={()=>setNeg({...neg,[i]:!neg[i]})} style={{width:24,height:24,borderRadius:5,border:"1px solid "+(neg[i]?"rgba(42,157,143,0.4)":"rgba(230,57,70,0.3)"),background:neg[i]?"rgba(42,157,143,0.12)":"rgba(230,57,70,0.08)",color:neg[i]?"#2A9D8F":"#E63946",fontSize:13,fontWeight:800,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{neg[i]?"−":"+"}</button>
          <input type="number" inputMode="numeric" placeholder="0" value={man[i]||""} onChange={e=>setMan({...man,[i]:e.target.value})} style={{flex:1,padding:4,borderRadius:5,border:"1px solid rgba(255,255,255,0.07)",background:"rgba(255,255,255,0.03)",color:neg[i]?"#2A9D8F":"#fff",fontSize:14,fontWeight:700,textAlign:"center",outline:"none",boxSizing:"border-box"}}/>
        </div>
      </div>)}</div></>}
...
export default function HandGame(){
  ...
  const submitRound=rd=>{
    ...
    const needs=p=>{const d=p.t-leader.t;if(d<=0)return null;if(d<=130)return p.name+" يحتاج فرش عادي";if(d<=260)return p.name+" يحتاج هند عادي أو فرش دبل";if(d<=520)return p.name+" يحتاج هند سوبر";if(d<=1040)return p.name+" يحتاج هند سوبر جوكر";return p.name+" صعب يلحق حتى بسوبر جوكر";};
