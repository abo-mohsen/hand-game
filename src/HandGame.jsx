    ...
    <div style={{width:"100%",maxWidth:420,maxHeight:"90vh",overflowY:"auto",background:"linear-gradient(135deg,#1a1a3e,#24243e)",borderRadius:18,padding:18,border:"1px solid rgba(255,255,255,0.1)",direction:"rtl",color:"#fff"}}>
      ...
      {type==="farsh"&&<>
        ...
        <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
          ...
          <button onClick={()=>setStolenP(null)} style={{padding:"5px 10px",borderRadius:7,border:"1px solid rgba(255,255,255,0.08)",background:stolenP===null&&khalasP!==null?"rgba(255,255,255,0.08)":"transparent",color:"rgba(255,255,255,0.5)",fontSize:10,cursor:"pointer"}}>ما أحد</button>
        </div>

        {/* 3. مين فرش بعد */}
        <L>مين فرش بعد؟</L>
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
          <button onClick={()=>{setExtraFarsh(!extraFarsh);if(extraFarsh)setExtraPs([]);}} style={{padding:"5px 12px",borderRadius:7,border:"1px solid "+(extraFarsh?"#F4A261":"rgba(255,255,255,0.08)"),background:extraFarsh?"rgba(244,162,97,0.12)":"transparent",color:extraFarsh?"#F4A261":"rgba(255,255,255,0.45)",fontSize:10,fontWeight:600,cursor:"pointer"}}>{extraFarsh?"نعم ✅":"لا"}</button>
        </div>
        {extraFarsh&&<div style={{background:"rgba(255,255,255,0.02)",borderRadius:8,padding:8,border:"1px solid rgba(255,255,255,0.04)"}}>
          ...
          {extraPs.length>0&&<div style={{display:"flex",flexDirection:"column",gap:4}}>
            {extraPs.map(ep=><div key={ep.i} style={{display:"flex",alignItems:"center",gap:6,background:"rgba(255,255,255,0.025)",borderRadius:7,padding:"6px 8px",border:"1px solid rgba(255,255,255,0.04)"}}>
              <div style={{display:"flex",alignItems:"center",gap:3,minWidth:65}}><div style={{width:6,height:6,borderRadius:"50%",background:players[ep.i].color}}/><span style={{fontSize:10,color:"rgba(255,255,255,0.6)"}}>{players[ep.i].name}</span></div>
              <input type="number" inputMode="numeric" placeholder="100" value={ep.score} onChange={e=>setExtraPScore(ep.i,e.target.value)} style={{flex:1,padding:"5px 4px",borderRadius:5,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",color:"#F4A261",fontSize:14,fontWeight:700,textAlign:"center",outline:"none",minWidth:0}}/>
            </div>)}
          </div>}
        </div>}
      </>}
      ...
      {type==="hand"&&<>
        ...
        <L>فيه أحد فرش بعد؟</L>
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
          <button onClick={()=>{setHandExtra(!handExtra);if(handExtra)setHandExtraPs([]);}} style={{padding:"5px 12px",borderRadius:7,border:"1px solid "+(handExtra?"#F4A261":"rgba(255,255,255,0.08)"),background:handExtra?"rgba(244,162,97,0.12)":"transparent",color:handExtra?"#F4A261":"rgba(255,255,255,0.45)",fontSize:10,fontWeight:600,cursor:"pointer"}}>{handExtra?"نعم ✅":"لا"}</button>
        </div>
        {handExtra&&<div style={{background:"rgba(255,255,255,0.02)",borderRadius:8,padding:8,border:"1px solid rgba(255,255,255,0.04)"}}>
          <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:6}}>
            {players.map((_,i)=>i!==handP&&<button key={i} onClick={()=>toggleHandExtraP(i)} style={{padding:"5px 10px",borderRadius:7,border:"1px solid "+(isHandExtraP(i)?players[i].color:"rgba(255,255,255,0.09)"),background:isHandExtraP(i)?players[i].color:"rgba(255,255,255,0.02)",color:isHandExtraP(i)?"#fff":"rgba(255,255,255,0.55)",fontSize:11,fontWeight:600,cursor:"pointer"}}>{players[i].name}{isHandExtraP(i)?" ✓":""}</button>)}
          </div>
          {handExtraPs.length>0&&<div style={{display:"flex",flexDirection:"column",gap:4}}>
            {handExtraPs.map(ep=><div key={ep.i} style={{display:"flex",alignItems:"center",gap:6,background:"rgba(255,255,255,0.025)",borderRadius:7,padding:"6px 8px",border:"1px solid rgba(255,255,255,0.04)"}}>
              <div style={{display:"flex",alignItems:"center",gap:3,minWidth:65}}><div style={{width:6,height:6,borderRadius:"50%",background:players[ep.i].color}}/><span style={{fontSize:10,color:"rgba(255,255,255,0.6)"}}>{players[ep.i].name}</span></div>
              <input type="number" inputMode="numeric" placeholder="نقاطه" value={ep.score} onChange={e=>setHandExtraPScore(ep.i,e.target.value)} style={{flex:1,padding:"5px 4px",borderRadius:5,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.04)",color:"#F4A261",fontSize:14,fontWeight:700,textAlign:"center",outline:"none",minWidth:0}}/>
            </div>)}
          </div>}
        </div>}
      </>}
      ...
      {(type!=="normal"||Object.values(man).some(v=>v))&&(()=>{
        ...
        return <div style={{background:"rgba(255,255,255,0.02)",borderRadius:8,padding:8,margin:"8px 0",border:"1px solid rgba(255,255,255,0.03)"}}>
          <div style={{fontSize:10,fontWeight:600,color:"rgba(255,255,255,0.4)",marginBottom:5}}>معاينة {handMode==="super"?"🔥":handMode==="superJoker"?"🃏🔥":""}</div>
          {pvSorted.map((p,rank)=>{const diff=p.newTotal-best;return <div key={p.i} style={{display:"flex",alignItems:"center",padding:"3px 0",gap:6}}>
            <span style={{fontSize:10,fontWeight:700,minWidth:14,textAlign:"center",color:rank===0?"#E9C46A":"rgba(255,255,255,0.4)"}}>{rank+1}</span>
            <div style={{width:5,height:5,borderRadius:"50%",background:p.color,flexShrink:0}}/>
            <span style={{fontSize:11,flex:1}}>{p.name}</span>
            <span style={{fontSize:12,fontWeight:700,color:p.score<0?"#2A9D8F":p.score>=200?"#E63946":"#fff",minWidth:40,textAlign:"center"}}>{p.score>0?"+":""}{p.score}</span>
            <span style={{fontSize:11,fontWeight:700,color:"rgba(255,255,255,0.5)",minWidth:35,textAlign:"center"}}>{p.newTotal}</span>
            {rank>0&&<span style={{fontSize:9,color:"rgba(230,57,70,0.7)",minWidth:30,textAlign:"left"}}>+{diff}</span>}
            {rank===0&&<span style={{fontSize:9,color:"#2A9D8F",minWidth:30,textAlign:"left"}}>👑</span>}
          </div>;})}
        </div>;
      })()}
